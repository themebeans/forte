	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php _e( 'Saving...', 'forte' ); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php _e( 'Create Gallery', 'forte' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php _e( 'Edit Gallery', 'forte' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php _e( 'Cancel', 'forte' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e( 'Edit Gallery', 'forte' ); ?>';
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php _e( 'Save Gallery', 'forte' ); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? esc_html__( 'Edit Gallery', 'forte' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = esc_html__( 'Edit Gallery', 'forte' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . esc_html__( 'Yes, please do', 'forte' ) . '</span>';
				echo '</td>';
				break;
		}
